...
float D_A = 1;  //0.2; 1;
float D_B = 0.5; //0.5; 
float f = 0.055; //0.00005; 0.055;
float k = 0.062; //0.15; 0.062;
float t = 1;
float pause = false;
...
{
    return (type?(0.2*(grid[x-1][y].A+grid[x][y+1].A+grid[x+1][y].A+grid[x][y-1].A) + 0.05*(grid[x-1][y-1].A+grid[x-1][y+1].A+grid[x+1][y+1].A+grid[x+1][y-1].A) - grid[x][y].A):(0.2*(grid[x-1][y].B+grid[x][y+1].B+grid[x+1][y].B+grid[x][y-1].B) + 0.05*(grid[x-1][y-1].B+grid[x-1][y+1].B+grid[x+1][y+1].B+grid[x+1][y-1].B) - grid[x][y].B));
}
...
void simulate()
{
    for(int row = 0; row<LEN_ROW-1; row++)
    {
        for(int column = 0; column<LEN_COLUMN-1; column++)
        {
...
void drawGrid()
{
    for(int row=0; row<LEN_ROW; row++)
        for(int column=0; column<LEN_COLUMN; column++)
                DrawRectangle(row*CELL_SIDE, column*CELL_SIDE, CELL_SIDE, CELL_SIDE, (Color){255-255*(grid[row][column].A-grid[row][column].B),255-255*(grid[row][column].A-grid[row][column].B),255-255*(grid[row][column].A-grid[row][column].B),255}); 
}
...
void mouseActions()
{
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT))
        ...
        grid[(int)(GetMouseX()/CELL_SIDE)+row][(int)(GetMouseY()/CELL_SIDE)+column].B = 1.0;
